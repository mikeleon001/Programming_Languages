...
procedure matdet is
 ...
 type MATRIX is array(NATURAL range <>, NATURAL range <>) of LONG_FLOAT;
	...
	m2: MATRIX (1..7,1..7);
	m3: MATRIX (1..7,1..7);
	m1Cofactor: MATRIX (1..7,1..7);
	m2Cofactor: MATRIX (1..7,1..7);
	m3Cofactor: MATRIX (1..7,1..7);
  ...
  --Procedure to populate Matrix (Reaed Long Float --> Populates Long Float)
	 ...
	 -- prints matrix to console (Reaed Long Float --> Prints string with 4 decimal places)
	 procedure PrintMatrixF4 (Input : in MATRIX) is
	 begin
		 Set_Output(Standard_Output);
		 for Row in Integer range 1..MAX_ROW loop
			 for Column in Integer range 1..MAX_COL loop
				 -- ugly format due to long ints and lack of width
				 Mat_input := Float(Input(Row, Column));
				 Put(Item => Mat_input, Aft => 4, Exp => 0);
...
--**************************************************
	...
	--****************************************************************
   --- calculating and printing the detarminanant of the input matrices

	Put_Line("Calculating the determinants of each matrices");
	Put_Line("*********************************************");


  	Put_Line("det(m1)");
	detm1 := Determinant(m1, 7);
	Put(Item => Float(detm1), Aft => 0, Exp => 0);
	Put_Line("");
	Put_Line("");

  	Put_Line("det(m2)");
	detm2 := Determinant(m2, 7);
	Put(Item => Float(detm2), Aft => 0, Exp => 0);
	Put_Line("");
	Put_Line("");
	...
	--*******************************************************************
   --- calculating and printing the cofactor matrix of the input matrices

	Put_Line("Calculating the Cofactor Matrix of each input matrix");
	Put_Line("****************************************************");

	Put_Line("Cofactor_Matrix(m1)");
	m1Cofactor := CofactoMatrix(m1);
	PrintMatrixLI(m1Cofactor);
	Put_Line("");
	
	Put_Line("Cofactor_Matrix(m2)");
	m2Cofactor := CofactoMatrix(m2);
	PrintMatrixLI(m2Cofactor);
	Put_Line("");

	Put_Line("Cofactor_Matrix(m3)");
	m3Cofactor := CofactoMatrix(m3);
	PrintMatrixLI(m3Cofactor);
	Put_Line("");
